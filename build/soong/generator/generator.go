...
func init() {
	android.RegisterModuleType("ligm_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = ligmExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = ligmExpandVariables(ctx, dep_file)
	...
	cmd := ligmExpandVariables(ctx, String(g.properties.Cmd))
